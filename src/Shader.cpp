...
void Shader::Load(const char* vertexPath, const char* fragmentPath) {
  ...
  glAttachShader(shaderProgramId_, vertex);
  glAttachShader(shaderProgramId_, fragment);
  glLinkProgram(shaderProgramId_);
  checkCompileErrors(shaderProgramId_, "PROGRAM");
...
void Shader::use() { glUseProgram(shaderProgramId_); }
...
void Shader::manipulateUniformFloat(const std::string& name,
                                    float& value) const {
  glUniform4f(glGetUniformLocation(shaderProgramId_, name.c_str()), 0.0f, value,
              0.0f, 1.0f);
}
